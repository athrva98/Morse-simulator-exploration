# engine speed controller initilization
# attitude controller
# this is the file for waypointInitialization
...
for i in range(startIndex,startIndex + max_robots_per_senv):
    ...
    motion = QuadrotorDynamicControl() # This is a publisher based on linear and angular velocity.

    motion.translate(z=0.0)

    motion.add_stream('socket', port = motion_port)
    config.motion_port = motion_port
    ...
    config.depthcamera_port = depthcamera_port
