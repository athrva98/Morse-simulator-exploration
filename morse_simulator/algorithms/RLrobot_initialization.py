...
with open(config.savePath + '/logs/robotServerPortCorrespondence.txt','a+') as rsc:
    print('Writing Robot Server correspondence')
...
for i in range(startIndex,startIndex + max_robots_per_senv):
    ...
    # videocamera_port = local_ports[0] # I leave this code intact for future uses.
    ...
    motion = Teleport() # This is a publisher based on waypoints.
    ...
    # motion = Waypoint() # This is the waypoint based controller.
    # motion.translate(z = 0.0)
    # motion.properties(FreeZ = True) # allows control in Z direction as well
    # motion.add_stream('socket', port = motion_port)
    # motion.add_service('socket')
